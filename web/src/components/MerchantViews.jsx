...
function CampaignDraft({ campaign }) {
  ...
  return <section className="campaign-draft"><header><div><span className={`draft-status ${blocked ? 'blocked' : ''}`}>{campaign.status}</span><h2>{campaign.campaign_name || 'Campaign review'}</h2><p>{campaign.goal}</p></div><div className="draft-date"><CalendarClock size={17} /> {campaign.duration_days ? `${campaign.duration_days} day duration` : 'Review required'}</div></header>
    {blocked ? <div className="policy-callout blocked"><CircleAlert size={19} /><p>{campaign.reason}</p></div> : <><div className="campaign-facts"><article><span>Opportunity detected</span><strong>{campaign.opportunity?.category}</strong><p>{campaign.opportunity?.attachment_rate}% attachment among demo laptop buyers</p></article><article><span>Target audience</span><strong>{campaign.target}</strong><p>Derived from the synthetic demo dataset</p></article><article><span>Offer</span><strong>{campaign.offer?.value}% off</strong><p>Maximum discount {formatINR(campaign.offer?.max_discount_inr)}</p></article></div>
      ...
      <CampaignEconomics economics={campaign.economics} />
...
function CampaignEconomics({ economics }) {
  if (!economics) return null
  const metrics = [
    ['Target customers', economics.target_customers],
    ['Expected orders', economics.expected_orders],
    ['Estimated revenue', formatINR(economics.estimated_revenue_inr)],
    ['Discount cost', formatINR(economics.discount_cost_inr)],
    ['Net incremental revenue', formatINR(economics.net_incremental_revenue_inr)],
    ['Estimated ROI', `${economics.estimated_roi_percent}%`],
  ]
  return <section className="campaign-economics"><div className="economics-heading"><div><span className="eyebrow accent"><BarChart3 size={14} /> Estimated campaign impact</span><h2>Conservative economics model</h2></div><span className="economics-label">Estimated using synthetic demo data</span></div><div className="economics-grid">{metrics.map(([label, value]) => <article key={label}><span>{label}</span><strong>{value}</strong></article>)}</div><div className="economics-assumptions"><div><CircleAlert size={17} /><strong>How this estimate is calculated</strong></div><ul>{economics.assumptions?.map((assumption) => <li key={assumption}>{assumption}</li>)}</ul></div></section>
}
...
export function AIBuyerView({ onRequest, quote, submittedGoal, loading }) {
